...
class ListHealthcareCandidates extends ListRecords implements HasForms
{
     ...
     * (HealthcareCandidatesTable — a bare, unfiltered company-wide list with
        ...
        return $table
            ...
            ->recordActions([
                ActionGroup::make([
                    ...
                    Action::make('setWeekAvailability')
                        ...
                        ->action(fn (array $data, HealthcareCandidate $record) => $this->applyStatusToUnknownWeekDays($record->id, $data['status'])),
                    Action::make('saveWeekAvailability')
                        ->label('Save')
                        ->icon('heroicon-o-check')
                        ->color('success')
                        ->visible(fn (HealthcareCandidate $record): bool => filled($this->pendingAvailability[$record->id] ?? null))
                        ->action(fn (HealthcareCandidate $record) => $this->saveStagedAvailability($record->id)),
                ])
            ...
            ->columns([
                TextColumn::make('name')
                    ->label('Name')
                    ->getStateUsing(fn (HealthcareCandidate $record): string => trim("{$record->first_name} {$record->last_name}"))
        ...
        return collect(CandidateSearchService::WEEKDAYS)
            ->map(function (int $isoWeekday) use ($weekStart): ViewColumn {
                ...
                return ViewColumn::make("day_{$isoWeekday}")
                    ->label($date->format('D'))
                    ->alignCenter()
                    ->view('filament.resources.healthcare-candidates.pages.partials.availability-day-cell')
                    ->getStateUsing(function (HealthcareCandidate $record) use ($date, $isoWeekday): array {
    ...
    {
        $candidate = HealthcareCandidate::query()->find($candidateId);
