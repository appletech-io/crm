        ...
        return $schema
            ...
            ->components([
                ...
                Section::make('Payroll Submission Failed')
                    ...
                    ->iconColor('danger')
                    ->visible(fn (?HealthcareCandidate $record): bool => $record && static::currentProviderErrors($record)->isNotEmpty())
                    ->schema([
                        Textarea::make('payroll_provider_errors')
                            ...
                            ->rows(3)
                            ->columnSpanFull()
                            ->afterStateHydrated(function (Textarea $component, ?HealthcareCandidate $record): void {
                                if ($record) {
                ...
                Tabs::make('Tabs')
                    ->tabs([
                        ...
                        Tab::make('Bookings')
                            ->visible(fn (?HealthcareCandidate $record): bool => $record && $record->bookings()->exists())
                            ->schema([
                        ...
                        Tab::make('Profile')
                            ...
                            ->schema([
                                ...
                                Text::make(fn (?HealthcareCandidate $record): ?string => $record?->candidateProfile?->updated_at
                                    ? 'Last generated: '.$record->candidateProfile->updated_at->diffForHumans()
                                    : null)
                                    ->visible(fn (?HealthcareCandidate $record): bool => $record?->candidateProfile?->updated_at !== null)
                                    ->color('gray'),
