...
test('geocoding job is dispatched when postcode is set on create', function () {
    ...
    $candidate = Candidate::factory()->create(['postcode' => 'SW1A 1AA']);

    Queue::assertPushed(GeocodeCandidate::class, fn ($job) => $job->candidate->is($candidate));
});

test('geocoding job is dispatched when postcode changes', function () {
    Queue::fake();

    $candidate = Candidate::factory()->create(['postcode' => null]);

    Queue::assertNotPushed(GeocodeCandidate::class);
...
test('geocoding job stores latitude and longitude from google response', function () {
    ...
    (new GeocodeCandidate($candidate))->handle();
...
test('geocoding job handles a failed google response gracefully', function () {
    ...
    $candidate = Candidate::factory()->create(['postcode' => 'SW1A 1AA', 'latitude' => null, 'longitude' => null]);
