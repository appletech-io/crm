<x-filament-panels::page>
    <div class="flex flex-col gap-6" wire:init="loadAll">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                <button
                    ...
                    @class([
                        'rounded-full px-3 py-1 text-xs font-medium transition',
                        'bg-green-600 text-white' => $activeTab === 'summary',
                        'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-white/5 dark:text-gray-400 dark:hover:bg-white/10' => $activeTab !== 'summary',
                    ])
                >
                    AI Summary
                </button>
                ...
                <button
                    type="button"
                    wire:click="setTab('bdm-calls')"
                    @class([
                        'rounded-full px-3 py-1 text-xs font-medium transition',
                        'bg-orange-600 text-white' => $activeTab === 'bdm-calls',
                        'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-white/5 dark:text-gray-400 dark:hover:bg-white/10' => $activeTab !== 'bdm-calls',
                    ])
                >
                    BDM Call Coaching
                </button>
        ...
        @else
            <div class="flex items-start gap-3 rounded-lg border border-orange-500/30 bg-orange-500/10 p-4 dark:border-orange-400/30 dark:bg-orange-400/10">
                <x-filament::icon
                    icon="heroicon-o-sparkles"
                    class="h-5 w-5 shrink-0 text-orange-600 dark:text-orange-400 {{ $bdmSummary === null ? 'animate-pulse' : '' }}"
                />

                <div class="flex flex-col gap-1">
                    <span class="text-xs font-semibold tracking-wide text-orange-700 uppercase dark:text-orange-400">
                        BDM Call Coaching
                    </span>

                    @if ($bdmSummary === null)
                        <p class="animate-pulse text-sm text-gray-500 dark:text-gray-400">
                            Reviewing this consultant's BDM and client calls…
                        </p>
                    @else
                        <p class="text-sm whitespace-pre-line text-gray-700 dark:text-gray-300">
                            {{ $bdmSummary }}
            ...
            <x-filament::section heading="BDM &amp; client calls logged">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead>
                            <tr class="text-xs text-gray-500 uppercase dark:text-gray-400">
                        ...
                        <tbody>
                            @forelse ($this->bdmCallActivities() as $call)
                                <tr class="border-t border-gray-100 dark:border-white/5">
                                    <td class="py-1.5 pr-4 whitespace-nowrap">{{ $call['created_at']->format('d M Y, H:i') }}</td>
                                    <td class="py-1.5 pr-4">
                                        <x-filament::badge :color="$call['type']->color()">
                                            {{ $call['type']->label() }}
                                        </x-filament::badge>
                                    </td>
                                    <td class="py-1.5 pr-4 whitespace-nowrap">{{ $call['subject'] }}</td>
                                    <td class="py-1.5 pr-4">{{ $call['note'] }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="py-3 text-sm text-gray-500 dark:text-gray-400">
                                        No BDM calls or client calls logged in this period.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
