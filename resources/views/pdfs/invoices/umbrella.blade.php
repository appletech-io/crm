...
<body>
    ...
    <footer>
        {{ $company->trading_name ?? $company->name }} &mdash; Self-Bill {{ $invoice->number }}
    </footer>

    <h1>Self-Bill Invoice {{ $invoice->number }}</h1>

    <p class="self-bill-notice">
        This is a self-billed invoice raised by {{ $company->trading_name ?? $company->name }} on behalf of
        {{ $provider->name }}, covering candidates paid via this umbrella company for the period below.
    </p>
    ...
    <table class="summary">
        <tr>
            ...
            <td>
                ...
                @if ($provider->address_1)
                    {{ $provider->address_1 }}<br>
                @endif
                {{ collect([$provider->county, $provider->postcode])->filter()->implode(', ') }}
                @if ($provider->vat_reg_number)
                    <br>VAT No: {{ $provider->vat_reg_number }}
                @endif
                @if ($provider->company_reg_number)
                    <br>Company No: {{ $provider->company_reg_number }}
                @endif
            </td>
        </tr>
        <tr>
    ...
    @if ($provider->bank_account_number)
        <p style="margin-top: 30px;">
            <strong>Payment Details (to {{ $provider->name }}):</strong><br>
            {{ $provider->bank_name }} &mdash; {{ $provider->bank_account_name }}<br>
            Account: {{ $provider->bank_account_number }} &mdash; Sort Code: {{ $provider->bank_sort_code }}
        </p>
    @endif
</body>
</html>
