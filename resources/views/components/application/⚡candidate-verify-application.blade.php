...
new #[Layout('layouts.application')] class extends Component
{
    ...
    public function mount(string $token): void
    {
        ...
        $this->application = CandidateApplication::where('token', $token)
            ->first();
        ...
        if (ApplicationAccessSession::hasVerified($token)) {
            $this->redirect(route('application.candidate.form', ['token' => $token]));
        }
    }
    ...
    public function verify(): void
    {
        ...
        if (strtolower($this->email) !== strtolower($this->application->candidate->email)) {
        ...
        $this->redirect(route('application.candidate.form', ['token' => $this->token]));
    }
};
