<div class="flex flex-col gap-6">
    ...
    {{-- Step 1: CV upload --}}
    @if ($currentStep === 1)
        @include('components.application.healthcare-application-form-steps.cv-upload')
    @endif

    {{-- Step 2: Personal details --}}
    @if ($currentStep === 2)
        @include('components.application.healthcare-application-form-steps.personal-details')
    @endif

    {{-- Step 3: Right to work & skills --}}
    @if ($currentStep === 3)
        @include('components.application.healthcare-application-form-steps.skills-work')
    @endif

    {{-- Step 4: Employment history --}}
    @if ($currentStep === 4)
        @include('components.application.healthcare-application-form-steps.employment-history')
    @endif

    {{-- Step 5: References --}}
    @if ($currentStep === 5)
        @include('components.application.healthcare-application-form-steps.references')
    @endif

    {{-- Step 6: Set password --}}
    @if ($currentStep === 6)
        @include('components.application.healthcare-application-form-steps.set-password')
    @endif
