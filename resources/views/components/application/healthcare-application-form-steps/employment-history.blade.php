        ...
        <div wire:key="employment-history-{{ $index }}" class="flex flex-col gap-4 rounded-lg border border-zinc-200 p-4 dark:border-white/10">
            ...
            @else
                <div class="grid grid-cols-2 gap-4">
                    <flux:input wire:model="employmentHistories.{{ $index }}.job_title" :label="__('Job Title')" placeholder="{{ __('Healthcare Assistant') }}" />
                    <flux:input wire:model="employmentHistories.{{ $index }}.company_name" :label="__('Company / Employer')" placeholder="{{ __('Oakwood Care Home') }}" />
                </div>
