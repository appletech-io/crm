...
<div wire:poll.60s class="flex min-h-screen flex-col items-center gap-10 px-8 py-12 text-center">
    <div>
        <p class="text-4xl font-semibold text-amber-300 sm:text-5xl">{{ $this->partOfDay }}</p>
        <h1 class="mt-2 text-6xl font-bold tracking-tight sm:text-8xl">{{ $this->today->format('l') }}</h1>
        <p class="mt-3 text-3xl text-neutral-300 sm:text-4xl">{{ $this->today->format('j F Y') }}</p>
        <p class="mt-6 text-5xl font-mono font-semibold sm:text-7xl" x-data x-init="
            $el.textContent = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'});
            setInterval(() => { $el.textContent = new Date().toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'}); }, 1000);
        "></p>
    </div>

    <div class="w-full max-w-3xl rounded-3xl bg-white/5 p-8 text-left">
        <h2 class="text-3xl font-semibold text-amber-300 sm:text-4xl">Today</h2>
        ...
        @else
            <ul class="mt-6 flex flex-col gap-5">
                @foreach ($this->todaysEvents as $event)
                    <li @class([
                        'flex items-baseline gap-6 rounded-2xl px-6 py-5 text-3xl sm:text-4xl',
                        'bg-amber-400 text-neutral-900 font-bold' => $event->id === $this->currentEventId,
                    ])>
    ...
    <div class="w-full max-w-3xl rounded-3xl bg-white/5 p-8 text-left">
        <h2 class="text-3xl font-semibold text-neutral-300 sm:text-4xl">Tomorrow — {{ $this->tomorrow->format('l') }}</h2>

        @if ($this->tomorrowsEvents->isEmpty())
            <p class="mt-6 text-2xl text-neutral-500">Nothing planned yet.</p>
        @else
            <ul class="mt-6 flex flex-col gap-4">
                @foreach ($this->tomorrowsEvents as $event)
                    <li class="flex items-baseline gap-6 text-2xl text-neutral-300 sm:text-3xl">
                        <span class="w-40 shrink-0 font-mono">{{ $event->display_time }}</span>
                        <span>{{ $event->title }}</span>
                    </li>
                @endforeach
